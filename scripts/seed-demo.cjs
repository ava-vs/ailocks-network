...
async function seedDemoData() {
  console.log('🌱 Starting enhanced demo database seeding with 50 intents...');
  ...
  try {
    ...
    const userMap = demoUsers.reduce((acc, user) => {
      if (user.name) {
    ...
    console.log(`✅ Created ${demoIntentsData.length} intents`);
    console.log('\n🎉 Enhanced demo database seeding completed successfully!');
    console.log('\n📊 Demo Data Summary:');
    console.log(`   👥 Users created: ${demoUsers.length}`);
    console.log(`   🎯 Intents created: ${demoIntentsData.length}`);
    console.log('\n🌟 Demo Scenario:');
    console.log('   • Lirea (designer in Lisbon) will search for opportunities and find Marco\'s in Rio.');
    console.log('   • The database is now populated with rich, global data to showcase the platform\'s power.');

    return true;

  } catch (error) {
    console.error('❌ Enhanced demo database seeding failed:', error.message);
    console.error('\n🔧 Troubleshooting:');
    console.error('   1. Verify DATABASE_URL is configured correctly');
    console.error('   2. Ensure database connection is working');
    console.error('   3. Check that database schema is up to date');
    console.error('   4. Run: npm run db:push to apply latest migrations');
...
if (require.main === module) {
  seedDemoData()
    .then(success => {
      if (success) {
        console.log('\n🚀 Ready for an amazing demo! Visit your app to see the new data.');
      }
